# services/pve-scripts-local-tcp3000.yaml
## Purpose
...
| `services[].asset_id` | `string` | `yes` | `yes` | `pve-scripts-local` | Asset where this service runs. |  |
...
| `services[].name` | `string` | `yes` | `yes` | `PVE Scripts Local` | Human-friendly service name. |  |
...
| `services[].ports.service_ports[].port` | `integer` | `yes` | `yes` | `3000` | TCP/UDP port. |  |
...
| `services[].service_id` | `string` | `yes` | `yes` | `pve-scripts-local-tcp3000` | Stable identifier for the service; often includes port/protocol. |  |

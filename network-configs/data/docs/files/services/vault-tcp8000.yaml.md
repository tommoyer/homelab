...
| `services[].asset_id` | `string` | `yes` | `yes` | `vault` | Asset where this service runs. |  |
...
| `services[].backend.port` | `integer` | `yes` | `yes` | `8000` | Backend port clients should connect to (often matches service_ports[].port). |  |
| `services[].interface_id` | `string` | `yes` | `yes` | `homelab` | Which interface on the asset should be used for backend addressing. |  |
| `services[].name` | `string` | `yes` | `yes` | `Vaultwarden` | Human-friendly service name. |  |
...
| `services[].ports.service_ports[].port` | `integer` | `yes` | `yes` | `8000` | TCP/UDP port. |  |
...
| `services[].service_id` | `string` | `yes` | `yes` | `vault-tcp8000` | Stable identifier for the service; often includes port/protocol. |  |

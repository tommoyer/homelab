...
| `services[].asset_id` | `string` | `yes` | `yes` | `sync` | Asset where this service runs. |  |
...
| `services[].backend.port` | `integer` | `yes` | `yes` | `8384` | Backend port clients should connect to (often matches service_ports[].port). |  |
| `services[].interface_id` | `string` | `yes` | `yes` | `homelab` | Which interface on the asset should be used for backend addressing. |  |
| `services[].name` | `string` | `yes` | `yes` | `Syncthing` | Human-friendly service name. |  |
...
| `services[].ports.service_ports` | `list` | `yes` | `yes` | `list(len=3)` | Ports the service listens on (authoritative). |  |
| `services[].ports.service_ports[]` | `list_item`, `object` | `yes` | `yes` | `list(len=3)`, `object(keys=2)` | One `service_port` entry (port/protocol tuple). |  |
| `services[].ports.service_ports[].port` | `integer` | `yes` | `yes` | `8384`, `22000` | TCP/UDP port. |  |
...
| `services[].service_id` | `string` | `yes` | `yes` | `sync-tcp8384` | Stable identifier for the service; often includes port/protocol. |  |

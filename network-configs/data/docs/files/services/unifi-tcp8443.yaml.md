...
| `services[].asset_id` | `string` | `yes` | `yes` | `unifi` | Asset where this service runs. |  |
...
| `services[].backend.port` | `integer` | `yes` | `yes` | `8443` | Backend port clients should connect to (often matches service_ports[].port). |  |
| `services[].interface_id` | `string` | `yes` | `yes` | `homelab` | Which interface on the asset should be used for backend addressing. |  |
| `services[].name` | `string` | `yes` | `yes` | `Unifi Controller` | Human-friendly service name. |  |
...
| `services[].ports.service_ports[].port` | `integer` | `yes` | `yes` | `8443` | TCP/UDP port. |  |
...
| `services[].service_id` | `string` | `yes` | `yes` | `unifi-tcp8443` | Stable identifier for the service; often includes port/protocol. |  |

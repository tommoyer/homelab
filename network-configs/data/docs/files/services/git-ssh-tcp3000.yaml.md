...
| `services[].asset_id` | `string` | `yes` | `yes` | `git-ssh` | Asset where this service runs. |  |
...
| `services[].backend.port` | `integer` | `yes` | `yes` | `3000` | Backend port clients should connect to (often matches service_ports[].port). |  |
| `services[].interface_id` | `string` | `yes` | `yes` | `homelab` | Which interface on the asset should be used for backend addressing. |  |
| `services[].name` | `string` | `yes` | `yes` | `Gitea (SSH)` | Human-friendly service name. |  |
...
| `services[].ports.firewall_ports[].port` | `integer` | `no` | `yes` | `3000`, `22` | Firewall-exposed port. |  |
...
| `services[].ports.service_ports[].port` | `integer` | `yes` | `yes` | `3000`, `22` | TCP/UDP port. |  |
...
| `services[].routing.internal_dns_target` | `string` | `no` | `yes` | `192.168.10.13` | When routing via Caddy, the internal A record target (often Caddy’s IP). | Only present when via_caddy=true in observed files. |
...
| `services[].service_id` | `string` | `yes` | `yes` | `git-ssh-tcp3000` | Stable identifier for the service; often includes port/protocol. |  |

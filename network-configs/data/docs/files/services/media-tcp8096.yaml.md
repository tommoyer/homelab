...
| `services[].asset_id` | `string` | `yes` | `yes` | `media` | Asset where this service runs. |  |
...
| `services[].backend.port` | `integer` | `yes` | `yes` | `8096` | Backend port clients should connect to (often matches service_ports[].port). |  |
| `services[].interface_id` | `string` | `yes` | `yes` | `homelab` | Which interface on the asset should be used for backend addressing. |  |
| `services[].name` | `string` | `yes` | `yes` | `Jellyfin` | Human-friendly service name. |  |
| `services[].ports` | `object` | `yes` | `yes` | `object(keys=2)` | Port definitions for the service. |  |
| `services[].ports.firewall_ports` | `list` | `yes` | `yes` | `list(len=1)` | Ports that should be exposed via firewall/reverse proxy (commonly 80/443 when using Caddy). |  |
| `services[].ports.firewall_ports[]` | `list_item`, `object` | `yes` | `yes` | `list(len=1)`, `object(keys=2)` | One `firewall_port` entry (port/protocol tuple). |  |
| `services[].ports.firewall_ports[].port` | `integer` | `no` | `yes` | `8096` | Firewall-exposed port. |  |
...
| `services[].ports.service_ports[].port` | `integer` | `yes` | `yes` | `8096` | TCP/UDP port. |  |
...
| `services[].service_id` | `string` | `yes` | `yes` | `media-tcp8096` | Stable identifier for the service; often includes port/protocol. |  |

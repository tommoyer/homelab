# services/minecraft-tcp25565.yaml
## Purpose
...
| `services[].asset_id` | `string` | `yes` | `yes` | `minecraft` | Asset where this service runs. |  |
...
| `services[].backend.port` | `integer` | `yes` | `yes` | `25565` | Backend port clients should connect to (often matches service_ports[].port). |  |
| `services[].interface_id` | `string` | `yes` | `yes` | `homelab` | Which interface on the asset should be used for backend addressing. |  |
| `services[].name` | `string` | `yes` | `yes` | `Minecraft` | Human-friendly service name. |  |
...
| `services[].ports.firewall_ports[].port` | `integer` | `no` | `yes` | `25565`, `19132` | Firewall-exposed port. |  |
| `services[].ports.firewall_ports[].proto` | `string` | `no` | `yes` | `tcp`, `udp` | Firewall-exposed protocol. |  |
| `services[].ports.service_ports` | `list` | `yes` | `yes` | `list(len=2)` | Ports the service listens on (authoritative). |  |
| `services[].ports.service_ports[]` | `list_item`, `object` | `yes` | `yes` | `list(len=2)`, `object(keys=2)` | One `service_port` entry (port/protocol tuple). |  |
| `services[].ports.service_ports[].port` | `integer` | `yes` | `yes` | `25565`, `19132` | TCP/UDP port. |  |
| `services[].ports.service_ports[].proto` | `string` | `yes` | `yes` | `tcp`, `udp` | Transport protocol. |  |
| `services[].routing` | `object` | `yes` | `yes` | `object(keys=2)` | How clients should reach the service. |  |
| `services[].routing.internal_dns_target` | `string` | `no` | `yes` | `192.168.10.10` | When routing via Caddy, the internal A record target (often Caddy’s IP). | Only present when via_caddy=true in observed files. |
| `services[].routing.via_caddy` | `boolean` | `yes` | `yes` | `false` | If true, service is routed via Caddy; DNS access names typically point at internal_dns_target. |  |
| `services[].service_id` | `string` | `yes` | `yes` | `minecraft-tcp25565` | Stable identifier for the service; often includes port/protocol. |  |

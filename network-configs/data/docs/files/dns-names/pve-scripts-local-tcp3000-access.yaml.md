# dns-names/pve-scripts-local-tcp3000-access.yaml
## Purpose
...
## Objects
- `pve-scripts-local-tcp3000-access`
...
| `dns_names[].dns_id` | `string` | `yes` | `yes` | `pve-scripts-local-tcp3000-access` | Stable identifier for the DNS record object. |  |
...
| `dns_names[].fqdn` | `string` | `yes` | `yes` | `pve-scripts-local.moyer.wtf` | Fully qualified domain name to publish. |  |
...
| `dns_names[].targets.service_id` | `string` | `yes` | `yes` | `pve-scripts-local-tcp3000` | Service target (mutually exclusive with asset_id). |  |

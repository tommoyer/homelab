# dns-names/sync-tcp8384-access.yaml
## Purpose
...
| `dns_names[].dns_id` | `string` | `yes` | `yes` | `sync-tcp8384-access` | Stable identifier for the DNS record object. |  |
...
| `dns_names[].fqdn` | `string` | `yes` | `yes` | `sync.moyer.wtf` | Fully qualified domain name to publish. |  |
...
| `dns_names[].targets.service_id` | `string` | `yes` | `yes` | `sync-tcp8384` | Service target (mutually exclusive with asset_id). |  |

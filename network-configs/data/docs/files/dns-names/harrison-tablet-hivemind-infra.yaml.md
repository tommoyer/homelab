# dns-names/harrison-tablet-hivemind-infra.yaml
## Purpose
...
## Objects
- `harrison-tablet-hivemind-infra`
...
| `dns_names[].dns_id` | `string` | `yes` | `yes` | `harrison-tablet-hivemind-infra` | Stable identifier for the DNS record object. |  |
...
| `dns_names[].fqdn` | `string` | `yes` | `yes` | `harrison-tablet.hivemind.moyer.wtf` | Fully qualified domain name to publish. |  |
...
| `dns_names[].targets.asset_id` | `string` | `yes` | `yes` | `harrison-tablet` | Asset target (mutually exclusive with service_id). |  |

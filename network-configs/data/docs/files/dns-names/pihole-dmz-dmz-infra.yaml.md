# dns-names/pihole-dmz-dmz-infra.yaml
## Purpose
...
| `dns_names[].dns_id` | `string` | `yes` | `yes` | `pihole-dmz-dmz-infra` | Stable identifier for the DNS record object. |  |
...
| `dns_names[].fqdn` | `string` | `yes` | `yes` | `pihole-dmz.dmz.moyer.wtf` | Fully qualified domain name to publish. |  |
...
| `dns_names[].internal.interface_id` | `string` | `no` | `yes` | `dmz` | When targeting an asset interface, which interface to use. | Only present when address=asset_interface_ip in observed files. |
...
| `dns_names[].targets.asset_id` | `string` | `yes` | `yes` | `pihole-dmz` | Asset target (mutually exclusive with service_id). |  |

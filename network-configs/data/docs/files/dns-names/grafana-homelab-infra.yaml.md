# dns-names/grafana-homelab-infra.yaml
## Purpose
...
| `dns_names[].dns_id` | `string` | `yes` | `yes` | `grafana-homelab-infra` | Stable identifier for the DNS record object. |  |
...
| `dns_names[].fqdn` | `string` | `yes` | `yes` | `grafana.homelab.moyer.wtf` | Fully qualified domain name to publish. |  |
...
| `dns_names[].targets.asset_id` | `string` | `yes` | `yes` | `grafana` | Asset target (mutually exclusive with service_id). |  |

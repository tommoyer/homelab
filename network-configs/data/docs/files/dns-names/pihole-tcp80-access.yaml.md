# dns-names/pihole-tcp80-access.yaml
## Purpose
...
| `dns_names[].dns_id` | `string` | `yes` | `yes` | `pihole-tcp80-access` | Stable identifier for the DNS record object. |  |
...
| `dns_names[].fqdn` | `string` | `yes` | `yes` | `pihole.moyer.wtf` | Fully qualified domain name to publish. |  |
...
| `dns_names[].targets.service_id` | `string` | `yes` | `yes` | `pihole-tcp80` | Service target (mutually exclusive with asset_id). |  |

# dns-names/media-tcp8096-access.yaml
## Purpose
...
| `dns_names[].dns_id` | `string` | `yes` | `yes` | `media-tcp8096-access` | Stable identifier for the DNS record object. |  |
...
| `dns_names[].fqdn` | `string` | `yes` | `yes` | `media.moyer.wtf` | Fully qualified domain name to publish. |  |
...
| `dns_names[].targets.service_id` | `string` | `yes` | `yes` | `media-tcp8096` | Service target (mutually exclusive with asset_id). |  |

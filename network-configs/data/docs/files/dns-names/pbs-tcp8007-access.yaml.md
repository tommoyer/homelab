# dns-names/pbs-tcp8007-access.yaml
## Purpose
...
| `dns_names[].dns_id` | `string` | `yes` | `yes` | `pbs-tcp8007-access` | Stable identifier for the DNS record object. |  |
...
| `dns_names[].fqdn` | `string` | `yes` | `yes` | `pbs.moyer.wtf` | Fully qualified domain name to publish. |  |
...
| `dns_names[].targets.service_id` | `string` | `yes` | `yes` | `pbs-tcp8007` | Service target (mutually exclusive with asset_id). |  |

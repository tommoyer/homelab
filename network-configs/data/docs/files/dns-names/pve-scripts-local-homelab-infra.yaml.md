# dns-names/pve-scripts-local-homelab-infra.yaml
## Purpose
...
## Objects
- `pve-scripts-local-homelab-infra`
...
| `dns_names[].dns_id` | `string` | `yes` | `yes` | `pve-scripts-local-homelab-infra` | Stable identifier for the DNS record object. |  |
...
| `dns_names[].fqdn` | `string` | `yes` | `yes` | `pve-scripts-local.homelab.moyer.wtf` | Fully qualified domain name to publish. |  |
...
| `dns_names[].internal.interface_id` | `string` | `no` | `yes` | `homelab` | When targeting an asset interface, which interface to use. | Only present when address=asset_interface_ip in observed files. |
...
| `dns_names[].targets.asset_id` | `string` | `yes` | `yes` | `pve-scripts-local` | Asset target (mutually exclusive with service_id). |  |

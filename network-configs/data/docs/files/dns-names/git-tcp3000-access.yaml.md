# dns-names/git-tcp3000-access.yaml
## Purpose
...
| `dns_names[].dns_id` | `string` | `yes` | `yes` | `git-tcp3000-access` | Stable identifier for the DNS record object. |  |
...
| `dns_names[].fqdn` | `string` | `yes` | `yes` | `git.moyer.wtf` | Fully qualified domain name to publish. |  |
...
| `dns_names[].targets.service_id` | `string` | `yes` | `yes` | `git-tcp3000` | Service target (mutually exclusive with asset_id). |  |

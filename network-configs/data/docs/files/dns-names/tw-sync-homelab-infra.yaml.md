# dns-names/tw-sync-homelab-infra.yaml
## Purpose
...
| `dns_names[].dns_id` | `string` | `yes` | `yes` | `tw-sync-homelab-infra` | Stable identifier for the DNS record object. |  |
...
| `dns_names[].fqdn` | `string` | `yes` | `yes` | `tw-sync.homelab.moyer.wtf` | Fully qualified domain name to publish. |  |
...
| `dns_names[].targets.asset_id` | `string` | `yes` | `yes` | `tw-sync` | Asset target (mutually exclusive with service_id). |  |

# dns-names/laptop-hivemind-infra.yaml
## Purpose
...
| `dns_names[].dns_id` | `string` | `yes` | `yes` | `laptop-hivemind-infra` | Stable identifier for the DNS record object. |  |
...
| `dns_names[].fqdn` | `string` | `yes` | `yes` | `laptop.hivemind.moyer.wtf` | Fully qualified domain name to publish. |  |
...
| `dns_names[].targets.asset_id` | `string` | `yes` | `yes` | `laptop` | Asset target (mutually exclusive with service_id). |  |

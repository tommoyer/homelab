# dns-names/tori-phone-hivemind-infra.yaml
## Purpose
...
| `dns_names[].dns_id` | `string` | `yes` | `yes` | `tori-phone-hivemind-infra` | Stable identifier for the DNS record object. |  |
...
| `dns_names[].fqdn` | `string` | `yes` | `yes` | `tori-phone.hivemind.moyer.wtf` | Fully qualified domain name to publish. |  |
...
| `dns_names[].targets.asset_id` | `string` | `yes` | `yes` | `tori-phone` | Asset target (mutually exclusive with service_id). |  |

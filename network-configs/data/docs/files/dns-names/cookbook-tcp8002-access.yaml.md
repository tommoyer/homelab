# dns-names/cookbook-tcp8002-access.yaml
## Purpose
...
| `dns_names[].cloudflare.proxy_status` | `string` | `no` | `yes` | `proxied` | Undocumented field (observed in repo). |  |
| `dns_names[].cloudflare.target_ip` | `string` | `no` | `yes` | `207.144.81.217` | Undocumented field (observed in repo). |  |
| `dns_names[].dns_id` | `string` | `yes` | `yes` | `cookbook-tcp8002-access` | Stable identifier for the DNS record object. |  |
...
| `dns_names[].external.proxied` | `boolean` | `no` | `yes` | `true` | Undocumented field (observed in repo). |  |
...
| `dns_names[].fqdn` | `string` | `yes` | `yes` | `cookbook.moyer.wtf` | Fully qualified domain name to publish. |  |
...
| `dns_names[].targets.service_id` | `string` | `yes` | `yes` | `cookbook-tcp8002` | Service target (mutually exclusive with asset_id). |  |

# dns-names/ledger-homelab-infra.yaml
## Purpose
...
| `dns_names[].dns_id` | `string` | `yes` | `yes` | `ledger-homelab-infra` | Stable identifier for the DNS record object. |  |
...
| `dns_names[].fqdn` | `string` | `yes` | `yes` | `ledger.homelab.moyer.wtf` | Fully qualified domain name to publish. |  |
...
| `dns_names[].targets.asset_id` | `string` | `yes` | `yes` | `ledger` | Asset target (mutually exclusive with service_id). |  |

# dns-names/clean-air-1-hivemind-infra.yaml
## Purpose
...
| `dns_names[].dns_id` | `string` | `yes` | `yes` | `clean-air-1-hivemind-infra` | Stable identifier for the DNS record object. |  |
...
| `dns_names[].fqdn` | `string` | `yes` | `yes` | `clean-air-1.hivemind.moyer.wtf` | Fully qualified domain name to publish. |  |
...
| `dns_names[].targets.asset_id` | `string` | `yes` | `yes` | `clean-air-1` | Asset target (mutually exclusive with service_id). |  |

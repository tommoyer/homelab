# dns-names/news-tcp8280-access.yaml
## Purpose
...
| `dns_names[].dns_id` | `string` | `yes` | `yes` | `news-tcp8280-access` | Stable identifier for the DNS record object. |  |
...
| `dns_names[].fqdn` | `string` | `yes` | `yes` | `news.moyer.wtf` | Fully qualified domain name to publish. |  |
...
| `dns_names[].targets.service_id` | `string` | `yes` | `yes` | `news-tcp8280` | Service target (mutually exclusive with asset_id). |  |

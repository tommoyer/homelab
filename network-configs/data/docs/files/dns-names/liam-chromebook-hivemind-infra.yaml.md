# dns-names/liam-chromebook-hivemind-infra.yaml
## Purpose
...
## Objects
- `liam-chromebook-hivemind-infra`
...
| `dns_names[].dns_id` | `string` | `yes` | `yes` | `liam-chromebook-hivemind-infra` | Stable identifier for the DNS record object. |  |
...
| `dns_names[].fqdn` | `string` | `yes` | `yes` | `liam-chromebook.hivemind.moyer.wtf` | Fully qualified domain name to publish. |  |
...
| `dns_names[].targets.asset_id` | `string` | `yes` | `yes` | `liam-chromebook` | Asset target (mutually exclusive with service_id). |  |

...
def process_vlan_mikrotik(vlan: str, dns_entries: List[Dict[str, Any]], firewall_entries: List[Dict[str, Any]], nat_entries: List[Dict[str, Any]], server_config: Dict[str, Any], global_config: Dict[str, Any], dry_run: bool, caddy_ip: Optional[str], wan_interface: str, router_host: Optional[str] = None, router_user: Optional[str] = None, router_port: Optional[str] = None, password: Optional[str] = None, keep_files: bool = False) -> None:
    ...
    # --- 2. Firewall Update ---
    # Check if we need to generate firewall rules (Caddy Pinholes OR NAT Forwarding)
    has_service_ports = any(e.get('ports') for e in firewall_entries)
    has_nat_entries = bool(nat_entries)
    
    if (caddy_ip and firewall_entries and has_service_ports) or has_nat_entries:
        ...
        # A. Caddy Pinholes
        if caddy_ip and firewall_entries and has_service_ports:
        ...
        elif has_service_ports and not caddy_ip:
             print(f"  [Warning] Service Ports found for VLAN {vlan}, but --caddy-ip was not provided. Skipping Caddy firewall rules.")

        # B. NAT Forwarding Rules
        if has_nat_entries:
            ...
            for e in nat_entries:
                ...
                if raw_ports and target_ip:
                    ...
                    if tcp_ports:
                        ...
                        rule_tcp = (
                            f'/ip firewall filter add chain=forward action=accept '
                            f'dst-address={target_ip} '
                            f'protocol=tcp dst-port={ports_joined} '
                            f'place-before=[find comment="Drop invalid/inter-VLAN forward"] '
                            f'comment="{fw_script_comment}"'
                        )
                        fw_lines.append(rule_tcp)
                        print(f"  [Firewall] Added NAT Forward TCP rule for {target_ip}: {ports_joined}")
                    ...
                    if udp_ports:
                        ...
                        fw_lines.append(rule_udp)
                        print(f"  [Firewall] Added NAT Forward UDP rule for {target_ip}: {ports_joined}")

...
def _render_commands(parsed: ParsedInput) -> list[str]:
    ...
    should_generate_filter = (
        parsed.enable_filter
        and parsed.backend_ip
        and (is_public or _should_generate_private_local_router_rules(parsed))
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    generated_public_caddy_service = False
    ...
    for service_key, form in prepared_services:
        ...
        if parsed.exposure == "public" and parsed.ingress == "caddy":
            generated_public_caddy_service = True
    ...
    if successes and caddy_ip_resolved and generated_public_caddy_service:
        caddy_dstnat_content = _render_caddy_dstnat(caddy_ip_resolved)
        caddy_dstnat_path = output_dir / "caddy-dstnat.rsc"
        caddy_dstnat_path.parent.mkdir(parents=True, exist_ok=True)
        caddy_dstnat_path.write_text(caddy_dstnat_content, encoding="utf-8")
